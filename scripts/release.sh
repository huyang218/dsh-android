...
NOTES=$(mktemp)
cat > "$NOTES" <<'NOTES_EOF'
非官方项目,不上架,安装需要打开"未知来源"。仅 arm64 设备。

apk 里带着 Node 二进制和 dsh 运行时快照,**装完就能用,不需要电脑**。第一次启动会
把它们解到应用私有目录(约 350 MB,几秒到十几秒,期间界面上会说明),之后每次启动
直接起 host。

手持形态**没有 shell**:agent 能读写文件、看附件、管会话和计划,但不执行命令。
为什么是这样,见 docs/feasibility.md。
